...
// validSecretRefNamePattern matches a valid ID or name for a vault, item, or field.
// Unsupported names can not be used in the secret reference syntax, and must be accessed
// by their native ID instead.
// https://www.1password.dev/cli/secret-reference-syntax#supported-characters
var validSecretRefNamePattern = regexp.MustCompile(`^[a-zA-Z0-9._-]+$`)

func isValidSecretRefName(str ...string) bool {
	for _, s := range str {
		if !isNativeID(s) && !validSecretRefNamePattern.MatchString(s) {
			return false
		}
	}
	return true
...
func (p *SecretsClient) getItem(ctx context.Context, name string) (onepassword.Item, error) {
	if cached, ok := p.Cache.GetItem(p.vaultID, name); ok {
...
// GetSecret returns a single secret from 1Password provider. If the vault/item/field names are supported
// per 1Password's secret reference syntax, we'll try to use the resolve API, which is more efficient.
// If not, we'll fall back to listing items and matching by name, which is less efficient but supports all valid 1Password names.
...
func (p *SecretsClient) GetSecret(ctx context.Context, ref esv1.ExternalSecretDataRemoteRef) ([]byte, error) {
	...
	// Fetch field/file from cache if it exists
	fieldCacheKey := p.createFieldCacheKey(ref.Key, ref.Property)
	if cached, ok := p.cacheGet(fieldCacheKey); ok {
		return cached, nil
	}
	fileCacheKey := p.createFileCacheKey(ref.Key, ref.Property)
	if cached, ok := p.cacheGet(fileCacheKey); ok {
		return cached, nil
	}

	// Fetch item from cache if it exists, and return the field if found
	itemCacheKey := p.createItemCacheKey(ref.Key)
	if cached, ok := p.cacheGet(itemCacheKey); ok {
		var item onepassword.Item
		if err := json.Unmarshal(cached, &item); err == nil {
			return p.getField(item, ref.Property)
		}
	}

	// If the secret reference fields are valid, resolve by secret reference
	if isValidSecretRefName(p.vaultID, ref.Key, ref.Property) {
...
// getField gets the first field of an item matching the given property label, or the field named "password" if `property` is not set.
func (p *SecretsClient) getField(item onepassword.Item, property string) ([]byte, error) {
	fieldLabel := defaultFieldLabel
	if property != "" {
		fieldLabel = property
	}

	// Throw error if there are multiple fields that match the given property label.
	if length := countFieldsWithLabel(fieldLabel, item.Fields); length != 1 {
		return nil, fmt.Errorf(errMsgExpectedOneField, fieldLabel, item.Title, length)
	}
...
func (p *SecretsClient) fetchFile(ctx context.Context, itemID string, fieldID string, attributes onepassword.FileAttributes) ([]byte, error) {
	...
	if err != nil {
		// TODO: improve error output and move fmtString to variable
...
// getFile gets the first file of an item matching the given property label, or the first file in the item if `property` is not set.
func (p *SecretsClient) getFile(ctx context.Context, item *onepassword.Item, property string) ([]byte, error) {
	if property != "" {
		// Throw error if there are multiple files that match the given property label.
		if length := countFilesWithLabel(property, item.Files); length != 1 {
	...
	for _, file := range item.Files {
		...
		if file.Attributes.Name == property || property == "" {
			return p.fetchFile(ctx, item.ID, file.FieldID, file.Attributes)
		}
	}

	return nil, fmt.Errorf(errMsgFileNotFound, property, item.Title)
}
...
func (p *SecretsClient) findItem(ctx context.Context, name string) (onepassword.Item, error) {
	if cached, ok := p.Cache.GetItem(name); ok {
		var item onepassword.Item
		if err := json.Unmarshal(cached, &item); err == nil {
			return item, nil
		}
